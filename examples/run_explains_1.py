...
def load_image_v1(base_image_path, all_images,
    ...
    for image_f in all_images:
        ...
        baseline_f = prefix_file_name + "_transformed.png"
...
def process_one_label(model_path, target_label, root_image_path, num_class,
                      target_size, normalized_factor, noise_type=None, noise_scale=None):
    ...
    transformed_folders = glob(image_path + "/transform_to_*/")
    for trans_f in transformed_folders:
        output_path = os.path.join(trans_f, "output")
        ...
        imag, base_imag, prefix_file_names = load_image_v1(trans_f,
                                                           all_images,
                                                           target_size,
                                                           normalized_factor)
        num_imag = imag.shape[0]
        print(f"Noise type: {noise_type}")
        if noise_type == "blur_0":
...
if __name__ == '__main__':
    ...
    root_image_path = "/home/ptien/from_ss/shap2/mnist/"
    explain_types = ["intgrad", "intgrad_base", "deeplift", "deeplift_base"]
    # target_size=[256, 256, 3]
    target_size=[28, 28, 1]
